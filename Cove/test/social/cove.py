...
train_val_fields = [
    ...
    ('social', LABEL) # process it as label
]
...
batch_size = 64 
list_scores=[]
f=open(path+'social_results.txt',mode='w+')
...
for epoch in range(1, epochs + 1):
    ...
    val_loss /= len(valds)
    torch.save(model.state_dict(), path + 'social_eval'+str(epoch)+'.pt')
...
model.load_state_dict(torch.load(path + 'social_eval'+str(ind+1)+'.pt'))
model.eval()
...
train.to_csv(path+"train_data2s.csv",index=False)
test.to_csv(path+"test_data2s.csv",index=False)
valid.to_csv(path+"valid_data2s.csv",index=False)